...
impl ExtAsset {
    ...
    pub fn from_internal_asset(internal_asset: &InternalAsset) -> Self {
        match internal_asset {
            InternalAsset::ft(ft_data) => ExtAsset::ft(ExtFTData {
                contract_id: ft_data.contract_id.clone(),
                registration_cost: U128(ft_data.registration_cost),
                tokens_per_use: U128(ft_data.tokens_transferred)
            })
        }
    }
}
...
impl ExtDrop {
    ...
    pub fn from_internal_drop(internal_drop: &InternalDrop) -> Self {
        ...
        for use_number in 1..=internal_drop.max_num_uses {
...
impl InternalAsset {
    ...
    pub fn from_ext_asset(ext_asset: &ExtAsset) -> Self {
        match ext_asset {
            ExtAsset::ft(ft_data) => InternalAsset::ft(InternalFTData::new(ft_data.contract_id, ft_data.registration_cost.into()))
        }
    }
...
pub struct InternalDrop {
    max_num_uses: UseNumber,
    asset_by_id: UnorderedMap<AssetId, InternalAsset>,
    asset_ids_by_use: LookupMap<UseNumber, Vec<AssetId>>,
}
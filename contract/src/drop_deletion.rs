...
impl Keypom {
    ...
    */
    pub fn delete_keys(
        ...
        // If drop config is delete empty
        let delete_delete_empty = drop.key_info_by_token_id.is_empty() && !keep_empty_drop.unwrap_or(false) && should_delete_on_empty;
        // If drop config is keep empty
        let delete_keep_empty = drop.key_info_by_token_id.is_empty() && !keep_empty_drop.unwrap_or(true) && !should_delete_on_empty;
        
        if delete_delete_empty || delete_keep_empty {

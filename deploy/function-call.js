...
async function start() {
	...
	try {
		...
		let args = btoa(JSON.stringify({
			...
			receiver_id: "benji-fayyr-linkdrop.testnet",
			token_metadata: METADATA,
		}));
        

		fc_data["receiver"] = "example-nft.testnet";
		fc_data["method"] = "nft_mint";
		fc_data["args"] = args;
		fc_data["deposit"] = parseNearAmount("1");
		...
		await fundingAccount.functionCall(
			...
			"300000000000000", 
			parseNearAmount((parseFloat(LINKDROP_NEAR_AMOUNT) + 2).toString())
		);
	} catch(e) {
		console.log('error initializing contract: ', e);
	}

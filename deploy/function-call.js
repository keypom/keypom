...
async function start() {
	...
	try {
		...
		for(var i = 1; i < keyPairs.length-1; i++) {
			...
			fc_data_final["deposit"] = parseNearAmount(((parseFloat(LINKDROP_NEAR_AMOUNT) + OFFSET + STORAGE) * (i+2)).toString());
			console.log("deposit for iter: ", i, " : ", ((parseFloat(LINKDROP_NEAR_AMOUNT) + OFFSET + STORAGE)) * (i+2).toString());
		...
		await fundingAccount.functionCall(
			...
			"300000000000000", 
			parseNearAmount(((parseFloat(LINKDROP_NEAR_AMOUNT) + OFFSET + STORAGE) * (keyPairs.length +2)).toString())
		);
	} catch(e) {
		console.log('error initializing contract: ', e);
	}

...
test('Create drop with 1 NFT (no owner)', async t => {
    ...
    await ali.call(keypom, 'create_drop', {public_keys: publicKeys, deposit_per_use: NEAR.parse('5 mN').toString(), config: {nft_metadata: {title: "My Cool NFT!", media: 'bafybeibwhlfvlytmttpcofahkukuzh24ckcamklia3vimzd4vkgnydy7nq'}}});
...
test('Create drop with 1 NFT (with owner)', async t => {
    ...
    const {publicKeys} = await generateKeyPairs(1);
    //create a drop with Ali, doesn't front any cost. 
    await ali.call(keypom, 'create_drop', {public_keys: publicKeys, key_owners: [ali.accountId], deposit_per_use: NEAR.parse('5 mN').toString(), config: {nft_metadata: {title: "My Cool NFT!", media: 'bafybeibwhlfvlytmttpcofahkukuzh24ckcamklia3vimzd4vkgnydy7nq'}}});

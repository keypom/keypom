...
test('Metadata and Royalties Helper Test', async t => {
    ...
    try{
        ...
        found_nft_info = await keypomV3.view("nft_token", {token_id: found_key_info.token_id})
        t.is(found_nft_info.approved_account_ids.hasOwnProperty(ali.accountId), true);
        t.is(found_nft_info.royalty[bob.accountId] == 500, true);
        ...
        let final_nft_token_info: {owner_id: string, royalty: Record<string, number>, metadata: TokenMetadata} = await keypomV3.view("nft_token", {token_id: found_key_info.token_id})
        ...
        console.log(royalties_and_metadata_same.royaltySame)
        console.log(royalties_and_metadata_same.metadataSame)

...
const test = anyTest as TestFn<{
  ...
  test.beforeEach(async (t) => {
    ...
    t.context.accounts = { root, funder, keypomV3, nftContract, nftContract2, ali };
...
test('Lots of Failed Claims', async t => {
    ...
    for (let i = 0; i < 5; i++) {
        if(i == 3){
            ...
            failed_token_ids.push(`token${i+2}`)
        }
    }
    
    // Tokens 1-6 exist and "belong" to Keypom
    await assertKeypomInternalAssets({
        keypom: keypomV3,
        dropId,
        expectedNftData: [{
    ...
    console.log(`Available Tokens: ${token_ids}`)
    ...
    let token: {token_id: string, owner_id: string} = await nftContract.view('nft_token', {token_id: "token1"});
    t.is(token.owner_id == claimingAccount, true)

    // Somehow, its registering the asset claim where I did not load the necessary tokens succeeded
    await assertKeypomInternalAssets({
        keypom: keypomV3,
        dropId,
        expectedNftData: [{
            contract_id: nftContract.accountId,
            token_ids: failed_token_ids
        },
        ],
    })
...
test('Non Asset Related Claim Fail - Assets Refunded', async t => {
    const {funder, keypomV3, nftContract, root, ali} = t.context.accounts;
    let initialBal = await keypomV3.balance();
